        ...
        if st.button("📊 Generate PDF", type="primary", use_container_width=True):
            with st.spinner("Creating PDF..."):
                try:
                    ...
                    # Determine page size and margins
                    ...
                    # Cover page
                    story_elements.append(Paragraph(st.session_state.book_title, styles['CoverTitle']))
                    story_elements.append(Paragraph(f"by {st.session_state.author_name}", styles['CoverAuthor']))
                    story_elements.append(Spacer(1, 2*inch))
                    story_elements.append(Paragraph(f"Generated on {datetime.now().strftime('%B %d, %Y')}", styles['Normal']))
                    
                    story_elements.append(PageBreak())
                    
                    # Table of Contents
                    if st.session_state.include_toc:
                        story_elements.append(Paragraph("Table of Contents", styles['Heading1']))
                        ...
                        if isinstance(stories, list):
                            ...
                            for i, story in enumerate(stories, 1):
                                ...
                                if session_id != current_session:
                                    session_title = story.get("session_title", f"Session {session_id}")
                                    story_elements.append(Paragraph(session_title, styles['Heading2']))
                                    current_session = session_id
                                question = story.get("question", f"Story {i}")
                                story_elements.append(Paragraph(f"{i}. {question}", styles['Normal']))
                        else:
                            for session_id, session_data in stories.items():
                                session_title = session_data.get("title", f"Session {session_id}")
                                story_elements.append(Paragraph(session_title, styles['Heading2']))
                                questions = session_data.get("questions", {})
                                for j, (question, _) in enumerate(questions.items(), 1):
                                    story_elements.append(Paragraph(f"{j}. {question}", styles['Normal']))
                    ...
                    if isinstance(stories, list):
                        ...
                        for i, story in enumerate(stories, 1):
                            ...
                            if session_id != current_session:
                                session_title = story.get("session_title", f"Session {session_id}")
                                story_elements.append(Paragraph(session_title, styles['ChapterTitle']))
                                current_session = session_id
                            ...
                            if st.session_state.selected_format == "interview":
                                story_elements.append(Paragraph(f"Q: {question}", styles['Question']))
                                story_elements.append(Paragraph(f"A: {answer}", styles['Answer']))
                            elif st.session_state.selected_format == "biography":
                                story_elements.append(Paragraph(answer, styles['Answer']))
                            else:  # memoir
                                story_elements.append(Paragraph(f"Chapter {i}: {question}", styles['ChapterTitle']))
                                story_elements.append(Paragraph(answer, styles['Answer']))
                        ...
                        for session_id, session_data in stories.items():
                            ...
                            for i, (question, answers) in enumerate(questions.items(), 1):
                                ...
                                if st.session_state.selected_format == "interview":
                                    story_elements.append(Paragraph(f"Q: {question}", styles['Question']))
                                    story_elements.append(Paragraph(f"A: {answer_text}", styles['Answer']))
                                elif st.session_state.selected_format == "biography":
                                    story_elements.append(Paragraph(answer_text, styles['Answer']))
                                else:  # memoir
                                    story_elements.append(Paragraph(f"Chapter {i}: {question}", styles['ChapterTitle']))
                                    story_elements.append(Paragraph(answer_text, styles['Answer']))
